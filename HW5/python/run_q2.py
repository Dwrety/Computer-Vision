    ...
    if '_' in k: 
        ...
        if 'b' in k:
            for i in range(params[k].shape[0]):
                params[k][i] += eps
                h_new1 = forward(x,params,'layer1')
                probs_new1 = forward(h_new1,params,'output',softmax)
                loss_new1, acc = compute_loss_and_acc(y, probs_new1)
                params[k][i] -= 2*eps
                h_new2 = forward(x,params,'layer1')
                probs_new2 = forward(h_new2,params,'output',softmax)
                loss_new2, acc = compute_loss_and_acc(y, probs_new2)
                params['grad_' + k][i] = (loss_new1 - loss_new2)/(2*eps)
                params[k][i] += eps
        else:
            for i in range(params[k].shape[0]):
                for j in range(params[k].shape[1]):
                    ...
                    probs_new2 = forward(h_new2,params,'output',softmax)
                    loss_new2, acc = compute_loss_and_acc(y, probs_new2)
                    params['grad_' + k][i,j] = (loss_new1 - loss_new2)/(2*eps)
                    params[k][i,j] += eps
...
# Backward to update params_orig
h_orig = forward(x,params_orig,'layer1')
probs_orig = forward(h_orig,params_orig,'output',softmax)
delta1_orig = probs_orig
delta1_orig[np.arange(probs_orig.shape[0]),y_idx] -= 1
delta2_orig = backwards(delta1_orig,params_orig,'output',linear_deriv)
backwards(delta2_orig,params_orig,'layer1',sigmoid_deriv)    

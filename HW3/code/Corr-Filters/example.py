...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        fig43, ax43 = plt.subplots(2,2)
        lamb = [0,1]
        S = X.dot(X.T)
        for l in lamb:
            S = S + l*np.eye(S.shape[0])
            g = nlg.inv(S).dot(X)
            g = g.dot(Y)
            g = g.reshape((dsize[0],dsize[1]))
            img43 = correlate(img,g)
            ax43[l,0].imshow(g)
            ax43[l,0].set_title('Correlation filter with Lambda = {}'.format(l))
            ax43[l,0].axis('off')
            ax43[l,1].imshow(img43, cmap='gray')
            ax43[l,1].set_title('Response with Lambda = {}'.format(l))
            ax43[l,1].axis('off')
        # plt.tight_layout()    
        # fig43.savefig("Q4_3COR.png",dpi=400)
        plt.show()
        return []
